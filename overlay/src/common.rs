...
const OPTION_FORCE_BUILTIN_SERVER: &str = "force-builtin-server";
...
#[inline]
pub fn get_update_artifact_prefix() -> String {
...
#[inline]
pub fn get_update_artifact_prefixes() -> Vec<String> {
    let primary = get_update_artifact_prefix();
    if primary == "rustdesk" {
        vec![primary]
    } else {
        vec![primary, "rustdesk".to_string()]
    }
}
...
#[inline]
fn is_server_core_locked() -> bool {
    get_builtin_option(OPTION_FORCE_BUILTIN_SERVER) == "Y"
}

#[inline]
fn get_runtime_server_override(key: &str) -> String {
    let opt_keys: &[&str] = match key {
        ...
        keys::OPTION_KEY => &["rdx-runtime-fixed-public-key"],
        _ => &[],
    };
    for opt_key in opt_keys {
        let value = LocalConfig::get_option(opt_key).trim().to_owned();
        if !value.is_empty() {
            return value;
        }
    }
...
pub fn get_custom_rendezvous_server(custom: String) -> String {
    let runtime = get_runtime_server_override(keys::OPTION_CUSTOM_RENDEZVOUS_SERVER);
    if !runtime.is_empty() {
        return runtime;
    }
    #[cfg(windows)]
    ...
    if is_server_core_locked() {
        let builtin = get_builtin_option(keys::OPTION_CUSTOM_RENDEZVOUS_SERVER);
        if !builtin.is_empty() {
            return builtin;
        }
...
fn get_api_server_(api: String, custom: String) -> String {
    let runtime = get_runtime_server_override(keys::OPTION_API_SERVER);
    if !runtime.is_empty() {
        return runtime;
    }
    #[cfg(windows)]
    ...
    if is_server_core_locked() {
        let api = get_builtin_option(keys::OPTION_API_SERVER);
        if !api.is_empty() {
            return api;
        }
        let builtin_custom = get_builtin_option(keys::OPTION_CUSTOM_RENDEZVOUS_SERVER);
        if !builtin_custom.is_empty() {
            let s = crate::increase_port(&builtin_custom, -2);
            if s == builtin_custom {
        ...
        return "https://admin.rustdesk.com".to_owned();
    }
    if !api.is_empty() {
        return api.to_owned();
    }
...
pub fn get_local_option(key: &str) -> String {
    if matches!(
        ...
        let runtime = get_runtime_server_override(key);
        if !runtime.is_empty() {
            return runtime;
        }
    }
...
pub async fn get_key(sync: bool) -> String {
    ...
    let runtime = get_runtime_server_override(keys::OPTION_KEY);
    if !runtime.is_empty() {
        return runtime;
    }
    if is_server_core_locked() {
        let mut key = get_builtin_option(keys::OPTION_KEY);
        if key.is_empty() {
...
pub fn read_custom_client(config: &str) {
    ...
    const KEY: &str = "JxfJbvG36fEHv/yVRdsP/Lse3mYiWTgWhZU3xFN+b30=";
