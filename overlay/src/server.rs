...
pub async fn create_tcp_connection(
    ...
    if secure && pk.len() == sign::PUBLICKEYBYTES && sk.len() == sign::SECRETKEYBYTES {
        log::info!(
            "create_tcp_connection secure handshake start: addr={}, conn_id={}",
            addr,
            id
        );
        ...
        log::info!(
            "create_tcp_connection sending SignedId to addr={}, conn_id={}",
            addr,
            id
        );
        ...
        match timeout(CONNECT_TIMEOUT, stream.next()).await? {
            Some(res) => {
                let bytes = res?;
                log::info!(
                    "create_tcp_connection received {} bytes from addr={}, conn_id={}",
                    bytes.len(),
                    addr,
                    id
                );
            ...
            None => {
                log::warn!(
                    "create_tcp_connection stream closed before PublicKey: addr={}, conn_id={}",
                    addr,
                    id
                );
...
const RELAY_FAST_FAIL_TIMEOUT_MS_DEFAULT: u64 = 3_500;

fn relay_fast_fail_timeout_ms() -> u64 {
    let fallback = std::cmp::min(CONNECT_TIMEOUT, RELAY_FAST_FAIL_TIMEOUT_MS_DEFAULT);
    Config::get_option("relay-fast-fail-timeout-ms")
        .parse::<u64>()
        .ok()
        .map(|v| v.clamp(500, CONNECT_TIMEOUT))
        .unwrap_or(fallback)
}

fn parse_relay_candidates(relay_server: &str) -> Vec<String> {
    let mut out = Vec::new();
    for x in relay_server
        .split(|c: char| c == ',' || c == ';' || c == '|' || c.is_ascii_whitespace())
    {
        let relay = x.trim();
        if relay.is_empty() || out.iter().any(|v| v == relay) {
            continue;
        }
...
async fn create_relay_connection_(
    ...
    let relay_candidates = parse_relay_candidates(&relay_server);
    if relay_candidates.is_empty() {
        bail!("No relay server candidates");
    }

    let fast_timeout_ms = relay_fast_fail_timeout_ms();
    let mut last_err = "unknown error".to_owned();
    for (i, relay) in relay_candidates.iter().enumerate() {
        if relay_candidates.len() > 1 {
            log::info!(
                "create relay candidate {}/{} for {}: {}",
                i + 1,
        ...
        let timeout_ms = if relay_candidates.len() > 1 && i + 1 < relay_candidates.len() {
            std::cmp::min(CONNECT_TIMEOUT, fast_timeout_ms)
        } else {
            CONNECT_TIMEOUT
        };

        let attempt = timeout(timeout_ms, async {
            let mut stream = socket_client::connect_tcp(
                socket_client::ipv4_to_ipv6(crate::check_port(relay.to_owned(), RELAY_PORT), ipv4),
                CONNECT_TIMEOUT,
            )
            .await?;
        ...
        match attempt {
            ...
            Ok(Err(e)) => {
                ...
                log::warn!("create relay candidate failed ({}): {}", relay, last_err);
            }
            Err(_) => {
                last_err = format!("timeout after {}ms", timeout_ms);
                log::warn!("create relay candidate timed out ({}): {}", relay, last_err);
            }
        }
    }

    bail!(
        "Failed to create relay connection via all relay candidates: {}",
        last_err
    );
}
...
pub async fn start_server(is_server: bool, no_server: bool) {
    ...
    if is_server {
        ...
        crate::client_control_agent::prime_runtime_bootstrap().await;
        crate::client_control_agent::maybe_start();
