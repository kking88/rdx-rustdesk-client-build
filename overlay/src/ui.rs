...
lazy_static::lazy_static! {
    ...
    static ref SERVICE_NODE_LATENCIES: Mutex<HashMap<String, i64>> = Default::default();
    static ref SERVICE_NODE_REFRESH_RUNNING: Mutex<bool> = Default::default();
}
...
fn normalize_service_node_pool_host(raw: &str) -> String {
    let trimmed = raw.trim();
    if trimmed.is_empty() {
        return String::new();
    }
    for sep in ['@', '#'] {
        if let Some((host, weight)) = trimmed.rsplit_once(sep) {
            if !host.trim().is_empty()
                && !weight.trim().is_empty()
                && weight.trim().chars().all(|c| c.is_ascii_digit())
            {
...
fn collect_service_node_pool() -> Vec<String> {
    use hbb_common::config::Config;

    let configured_pool = Config::get_option("rendezvous-servers");
    let mut pool_nodes: Vec<String> = configured_pool
        .split(',')
        .map(normalize_service_node_pool_host)
        .filter(|item| !item.is_empty())
        .collect();
    if pool_nodes.is_empty() {
        pool_nodes = Config::get_rendezvous_servers();
    }

    let manual_node = Config::get_option("relay-server");
    if !manual_node.trim().is_empty() && !pool_nodes.iter().any(|item| item == &manual_node) {
        pool_nodes.push(manual_node);
    }
    pool_nodes
}

fn probe_service_node_latency(host: &str) -> i64 {
    use hbb_common::config;

    let target = crate::check_port(host, config::RENDEZVOUS_PORT);
    ...
    for addr in addrs {
        ...
        if TcpStream::connect_timeout(&addr, timeout).is_ok() {
            return started_at.elapsed().as_millis() as i64;
        }
    }
    -1
}

fn refresh_service_node_probes() {
    let mut running = SERVICE_NODE_REFRESH_RUNNING.lock().unwrap();
    if *running {
        return;
    }
    *running = true;
    ...
    let hosts = collect_service_node_pool();
    std::thread::spawn(move || {
        ...
        for host in hosts {
            ...
            next.insert(normalized.clone(), probe_service_node_latency(&normalized));
        }
        *SERVICE_NODE_LATENCIES.lock().unwrap() = next;
        *SERVICE_NODE_REFRESH_RUNNING.lock().unwrap() = false;
    });
}

fn get_service_nodes() -> Value {
    use hbb_common::config::{self, Config};

    fn normalize_pool_host(raw: &str) -> String {
        let trimmed = raw.trim();
        if trimmed.is_empty() {
            return String::new();
        }
    ...
    fn push_node(
        ...
        if host.is_empty() || seen.iter().any(|item| item == &host) {
            return;
        }
        seen.push(host.clone());
        let latency_us = latencies.get(&host).copied().unwrap_or(0);
        let latency_ms = if latency_us > 0 {
            ((latency_us + 999) / 1000) as i32
        } else {
            latency_us as i32
        };
        let host_with_port = crate::check_port(&host, config::RENDEZVOUS_PORT);
        let mut item = Value::map();
        item.set_item("host", host.clone());
        item.set_item("hostWithPort", host_with_port.clone());
        item.set_item("latencyMs", latency_ms);
        item.set_item("isOnline", Value::from(latency_us > 0));
        item.set_item(
            "isCurrent",
            Value::from(host_with_port == current_node || host == current_node),
        );
        item.set_item(
            "isManual",
            Value::from(!manual_node.is_empty() && host == manual_node),
        );
        nodes.push(item);
    }

    let manual_node = Config::get_option("relay-server");
    let current_node = manual_node.clone();
    let latencies = SERVICE_NODE_LATENCIES.lock().unwrap().clone();

    let configured_pool = Config::get_option("rendezvous-servers");
    let mut pool_nodes: Vec<String> = configured_pool
        .split(',')
        .map(normalize_pool_host)
        .filter(|item| !item.is_empty())
        .collect();
    let pool_configured = !pool_nodes.is_empty();
    if pool_nodes.is_empty() {
        pool_nodes = Config::get_rendezvous_servers();
    }

    let mut nodes = Value::array(0);
    let mut seen = Vec::new();
    for host in pool_nodes {
        push_node(
            &mut nodes,
            &mut seen,
            &latencies,
    ...
    result.set_item("manualNode", manual_node);
    result.set_item("currentNode", current_node);
    result.set_item("poolConfigured", Value::from(pool_configured));
...
impl UI {
    ...
    fn get_service_nodes(&self) -> Value {
        get_service_nodes()
    }

    fn refresh_service_nodes(&self) {
        refresh_service_node_probes();
    ...
    fn http_request_async(&self, url: String, method: String, body: String, header: String) {
        let body = if body.is_empty() { None } else { Some(body) };
        http_request(url, method.to_lowercase(), body, header)
    }

    fn get_http_request_status(&self, url: String) -> String {
...
impl sciter::EventHandler for UI {
    sciter::dispatch_script_call! {
        ...
        fn http_request_async(String, String, String, String);
        fn get_http_request_status(String);
